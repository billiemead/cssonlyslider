    ...
    .then(data => {
      data.forEach(function(image, i) {
        document.querySelector("#neoscape-slide-" + (i+1)).innerHTML = `
          <img src="${image.urls.regular}" alt="" class="unsplash-img">
          <p class="unsplash-author-info">
            <a href="${image.links.html}">Photo by ${image.user.name}</a> on <a href="https://unsplash.com/" target="_blank">Unsplash</a>
          </p>
        `;
      });
    });